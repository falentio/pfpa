...
export class CollectionService {
	...
	private get metadataStorage() {
		return prefixStorage<Record<string, string>>(this.storage, "metadata");
	}
	...
	async getInfo(collectionName: string, id: number) {
		...
		const metadata = await this.metadataStorage.getItem(
			`${collectionName}:${id}`,
		);
		if (!metadata) {
			throw new FriendlyError({
	...
	async getImage(
		...
		await Promise.all([
			this.r2.put(`${collectionName}/${id}`, bytes, {
				httpMetadata: {
					contentType: created.image.contentType,
				},
			}),
			this.metadataStorage.setItem(`${collectionName}:${id}`, {
